...
class MetricsTracker:
    ...
    def log_training(self, epoch: int, loss: float, accuracy: float = 0.0,
                    belief_accuracy: float = 0.0, action_accuracy: float = 0.0,
    ...
    def log_evaluation(self, benchmark_scores: Dict, tom_order_scores: Dict,
                      cooperation_score: float, zombie_detection_score: float,
                      overall_score: float):
    ...
    def get_evaluation_summary(self) -> Dict:
        ...
        return {
            ...
            if len(overall_scores) > 1 else 0.0
        }
    ...
    def load_from_file(self, filepath: str):
        ...
        self.training_history = [
            TrainingMetrics(**m) for m in data.get('training_history', [])
        ]

        # Reconstruct evaluation history
        self.evaluation_history = [
            EvaluationMetrics(**m) for m in data.get('evaluation_history', [])
        ]
...
class PerformanceAnalyzer:
    ...
    @staticmethod
    def detect_overfitting(training_losses: List[float],
    ...
    def compare_architectures(results1: Dict, results2: Dict) -> Dict:
        ...
        comparison = {
            'architecture1_better': 0,
            'architecture2_better': 0,
            'tied': 0,
            'differences': {}
        }
    ...
    @staticmethod
    def calculate_efficiency_score(accuracy: float, num_parameters: int,
...
class ResultsAggregator:
    ...
    def get_best_run(self, metric_name: str, higher_is_better: bool = True) -> Optional[Dict]:
        ...
        valid_runs = [
            run for run in self.run_results
            if metric_name in run['results']
        ]
        ...
        if higher_is_better:
            best_run = max(valid_runs,
                          key=lambda x: x['results'][metric_name])
        else:
            best_run = min(valid_runs,
                          key=lambda x: x['results'][metric_name])

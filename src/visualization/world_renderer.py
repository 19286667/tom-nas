...
class WorldRenderer:
    ...
    def render_realm(self, realm_type: str = None) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=y_positions,
                mode="markers+text",
                marker=dict(size=15, color=colors, line=dict(width=2, color="white"), opacity=0.8),
                text=names,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[player_y],
                mode="markers",
                marker=dict(size=20, color="#FF0000", symbol="star", line=dict(width=2, color="black")),
                name="Player",
                hovertext="Player",
            ),
            row=1,
    ...
    def render_soul_map_radar(self, soul_map: "SoulMap") -> go.Figure:
        ...
        fig.update_layout(
            polar=dict(radialaxis=dict(visible=True, range=[0, 1])), showlegend=False, title="Soul Map Profile"
        )
    ...
    def render_npc_details(self, npc: "BaseNPC") -> go.Figure:
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            specs=[[{"type": "polar"}, {"type": "bar"}], [{"type": "scatter", "colspan": 2}, None]],
        ...
        fig.add_trace(
            go.Scatterpolar(r=values + [values[0]], theta=categories + [categories[0]], fill="toself", name="Profile"),
            row=1,
            col=1,
        )
        ...
        if hasattr(soul_map, "motivational"):
            ...
            fig.add_trace(go.Bar(x=drives, y=drive_values, name="Drives", marker_color="lightblue"), row=1, col=2)

        # Emotional state indicator (placeholder)
        fig.add_trace(
            go.Scatter(x=[0, 1, 2, 3, 4], y=[0.5, 0.6, 0.4, 0.7, 0.5], mode="lines+markers", name="Emotional Trend"),
            row=2,
            col=1,
        )

        fig.update_layout(title=f"NPC Details: {getattr(npc, 'name', npc.npc_id)}", height=600)
    ...
    def render_realm_overview(self) -> go.Figure:
        ...
        fig = go.Figure(data=[go.Bar(x=realms, y=counts, marker_color=colors, text=counts, textposition="auto")])

        fig.update_layout(title="NPC Distribution by Realm", xaxis_title="Realm", yaxis_title="Number of NPCs")
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    ...
                    marker_colors=[self.EMOTION_COLORS.get(e, "#808080") for e in emotion_counts.keys()],
                )
            ]
        )

        fig.update_layout(title=f"Emotional State Distribution{f' - {realm_type}' if realm_type else ''}")
    ...
    def _get_npcs_in_realm(self, realm_type: str) -> List["BaseNPC"]:
        ...
        return [npc for npc in self.env.npcs.values() if getattr(npc, "current_realm", None) == target_realm]
    ...
    def _create_empty_figure(self, message: str) -> go.Figure:
        ...
        fig = go.Figure()
        fig.add_annotation(text=message, xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font=dict(size=20))

...
class NASDashboard:
    ...
    def render_tom_specificity(self) -> go.Figure:
        ...
        fig.add_hline(y=0, line_dash="dot", line_color="red",
                     annotation_text="No ToM advantage")
    ...
    def render_feature_importance(self) -> go.Figure:
        ...
        for feature in features:
            ...
            if len(set(values)) > 1:  # Only if there's variation
                ...
                numerator = sum((v - mean_val) * (f - mean_fit)
                               for v, f in zip(values, fitnesses))
                denominator = (sum((v - mean_val)**2 for v in values) *
                              sum((f - mean_fit)**2 for f in fitnesses)) ** 0.5
        ...
        fig = go.Figure(data=[
            go.Bar(
                x=features,
                y=correlations,
                marker_color=['green' if c > 0.5 else 'orange' if c > 0.3 else 'gray'
    ...
    def render_zero_cost_proxy_correlations(self,
                                             proxy_results: Optional[List[Dict]] = None) -> go.Figure:
        """
        ...
        fig = make_subplots(rows=1, cols=3,
                           subplot_titles=['SynFlow', 'Jacobian Cov', 'Grad Norm'])
        ...
        fig.update_layout(
            title="Zero-Cost Proxy Correlations with Trained Fitness",
            height=350,
            showlegend=False
        )

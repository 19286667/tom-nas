...
class SocialWorld4:
    ...
    def play_cooperation_game(self, agent1_id: int, agent2_id: int, action1: str, action2: str) -> Dict:
        ...
        if action1 == "cooperate":
            self.agents[agent2_id].reputation[agent1_id] = min(1.0, self.agents[agent2_id].reputation[agent1_id] + 0.1)
        else:
            self.agents[agent2_id].reputation[agent1_id] = max(0.0, self.agents[agent2_id].reputation[agent1_id] - 0.1)

        if action2 == "cooperate":
            self.agents[agent1_id].reputation[agent2_id] = min(1.0, self.agents[agent1_id].reputation[agent2_id] + 0.1)
        else:
            self.agents[agent1_id].reputation[agent2_id] = max(0.0, self.agents[agent1_id].reputation[agent2_id] - 0.1)
    ...
    def play_communication_game(
        self, sender_id: int, receiver_id: int, message: torch.Tensor, true_state: torch.Tensor
    ) -> Dict:
        ...
        self.agents[receiver_id].reputation[sender_id] = (
            0.9 * self.agents[receiver_id].reputation[sender_id] + 0.1 * message_quality
        )
    ...
    def play_resource_sharing_game(self, giver_id: int, receiver_id: int, amount: float) -> Dict:
        ...
        if amount > 0:
            ...
            self.agents[receiver_id].reputation[giver_id] = min(
                1.0, self.agents[receiver_id].reputation[giver_id] + amount * 0.01
            )
    ...
    def attempt_zombie_detection(self, detector_id: int, suspect_id: int) -> Dict:
        ...
        detected_correctly = (is_zombie and detection_accuracy < 0.3) or (not is_zombie and detection_accuracy > 0.7)
    ...
    def step(self, agent_actions: List[Dict], belief_network=None) -> Dict:
        ...
        for i, action_dict in enumerate(agent_actions):
            ...
            if action_type == "cooperate" or action_type == "defect":
                ...
                if partner is None:
                    partner = random.choice([j for j in range(self.num_agents) if j != i and self.agents[j].alive])
                ...
                results["coalition_changes"].append(
                    {"action": "form", "coalition_id": coalition_id, "members": members}
                )

            elif action_type == "leave_coalition":
                self.leave_coalition(i)
                results["coalition_changes"].append({"action": "leave", "agent_id": i})

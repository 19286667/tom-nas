...
class ToMFitnessEvaluator:
    ...
    def evaluate_agent(
        self, agent_model: nn.Module, num_episodes: int = 10, episode_length: int = 50
    ) -> Dict[str, float]:
        """
        ...
        total_fitness = sum(self.weights.get(key, 0.0) * value for key, value in fitness_components.items())
    ...
    def _run_episode(self, agent_model: nn.Module, episode_length: int) -> Dict[str, float]:
        ...
        for t in range(episode_length):
            ...
            all_actions = [action if i == agent_id else {"type": "cooperate"} for i in range(self.world.num_agents)]
    ...
    def _observation_to_tensor(self, obs: Dict) -> torch.Tensor:
        ...
        features = [obs["own_resources"] / 200.0, obs["own_energy"] / 100.0, float(obs["own_coalition"] is not None)]
    ...
    def _beliefs_to_action(self, beliefs: torch.Tensor, action_value: torch.Tensor, obs: Dict) -> Dict:
        ...
        if action_value.item() > 0.7:
            ...
            return {"type": "detect_zombie", "suspect": obs["observations"][0]["id"]}
        else:
            return {"type": "share", "receiver": obs["observations"][0]["id"], "amount": 5.0}

    def _update_metrics(
        self, metrics: Dict, result: Dict, beliefs: torch.Tensor, agent_id: int, action_history: List
    ) -> None:
    ...
    def evaluate(self, agent_model: nn.Module) -> float:
        ...
        with torch.no_grad():
            ...
            initial_state = torch.tensor(
                [[1.0, 0.0, 1.0, 1.0, 1.0]], dtype=torch.float32  # marble in basket, both present
            ).unsqueeze(0)

            sally_leaves = torch.tensor(
                [[1.0, 0.0, 0.0, 1.0, 1.0]], dtype=torch.float32  # marble in basket, only Anne present
            ).unsqueeze(0)

            anne_moves_marble = torch.tensor(
                [[0.0, 1.0, 0.0, 1.0, 0.0]], dtype=torch.float32  # marble in box, only Anne present
            ).unsqueeze(0)

            sally_returns = torch.tensor(
                [[0.0, 1.0, 1.0, 1.0, 0.0]], dtype=torch.float32  # marble in box, both present
            ).unsqueeze(0)

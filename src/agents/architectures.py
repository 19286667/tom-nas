    ...
    def forward(self, x: torch.Tensor, hidden: Optional[torch.Tensor] = None):
        ...
        for t in range(seq_len):
            ...
            for layer_idx, layer in enumerate(self.layers):
                ...
                candidate_input = torch.cat([x_t, r * h], dim=1)
                h_tilde = torch.tanh(layer['candidate'](candidate_input))
        ...
        beliefs = torch.sigmoid(
            self.belief_projection(output_tensor[:, -1, :]))
        actions = torch.sigmoid(
            self.action_projection(output_tensor[:, -1, :]))
